...
if ($stmt = $con->prepare('SELECT id, password FROM facultystaffaccounts WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO facultystaffaccounts (username, password, email) VALUES (?, ?, ?)')) {
