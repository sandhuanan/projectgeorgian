...
if ($stmt = $con->prepare('SELECT id, password FROM communityaccounts WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO communityaccounts (username, password, email) VALUES (?, ?, ?)')) {
